 ...
 * una lista di vincoli (`constraintsArray`) e l'elenco delle misure richieste (`requestedMeasures`)
 * tramite POST.
 *
 ...
 *    `{ constraints: string[], requested_measures: string[] }`
 * 4. Aggiorna gli stati:
 *    - `summary`: risultato del backend (solo le misure richieste)
 ...
 * @returns {?Object} return.summary - Risultato del calcolo (solo misure richieste), oppure null.
 ...
 * @returns {Function} return.computeMeasures - Funzione per avviare il calcolo: `computeMeasures(constraintsArray, requestedMeasures)`.
 */
export default function useMeasures(selectedInstance) {
  ...
  function computeMeasures(constraintsArray, requestedMeasures) {
    if (!selectedInstance) {
    ...
    fetch(`${API_BASE}/api/measures/compute`, {
      ...
      body: JSON.stringify({
        ...
        requested_measures: requestedMeasures, 
      }),
    })
      .then(async (r) => {
        if (!r.ok) {
